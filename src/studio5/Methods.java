...
public class Methods {
	...
	public static String substituteAll(String source, char target, String replacement) {
		...
		boolean [] indexes = new boolean[source.length()];
		for (int i = 0; i < source.length(); i++ ) {
			char currentVariable = source.charAt(i);
			if (currentVariable == target) {
				indexes[i] = true;
			}
			else {
				indexes[i] = false;
		...
		char [] newsource = toCharArray(source)

		for (int a = 0; a < source.length(); a++) {
			if (indexes[a] == true) {
				
				  source.toCharArray[a] = replacement;
			}
		}
		}
		result = source;
